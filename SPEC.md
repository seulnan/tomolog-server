...
### Room
- `name`, `hostUserId` (null for THEMED rooms — no human host)
- `type` (enum: PRIVATE, THEMED) — see LEDGER-008
  - **PRIVATE**: host-created for people who know each other, host sets `capacity` (2~50)
  - **THEMED**: fixed set of 5 public "places" (분위기 좋은 카페, 제주 바다속 카페 등) where
    anonymous people study together at large scale, `capacity` up to 2000, seeded via Flyway
- `capacity`
